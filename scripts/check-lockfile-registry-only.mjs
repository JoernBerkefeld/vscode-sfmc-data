...
function scanPackages(packages, findings) {
    for (const [packagePath, node] of Object.entries(packages)) {
        ...
        for (const dependencyMap of ['dependencies', 'devDependencies', 'optionalDependencies', 'peerDependencies']) {
            const dependencies = node[dependencyMap];
            if (dependencies && typeof dependencies === 'object') {
                for (const [name, spec] of Object.entries(dependencies)) {
                    if (isLocalSpec(spec)) {
                        findings.push(`local ${dependencyMap} spec at packages["${packagePath}"].${name}: ${spec}`);
                    }
                }
            }
        }
...
function main() {
    ...
    if (findings.length > 0) {
        ...
        console.error("Fix package.json (use registry semver ranges), delete node_modules, then run 'npm install --no-workspaces'.");
        process.exitCode = 1;
    }
}
